...
function renderSelectionControl(
  ...
  const cannotAdd = addDisabled && !isSelected;
  const label = isSelected
    ? 'Retirer de ma sélection'
    : cannotAdd
      ? 'Ajout impossible — prix non indiqué'
      : 'Ajouter à ma sélection';
  const icon = isSelected ? ICON_CHECK : ICON_PLUS;
  const selectedClass = isSelected ? ' is-selected' : '';
  const disabledAttr = cannotAdd ? ' disabled' : '';
  ...
  return `
    <button
      ...
      aria-pressed="${isSelected}"${disabledAttr}
    >
      ${icon}
    </button>
  `;
...
function buildRowSelection(
  ...
  return {
    actionsHtml: `<div class="wine-row__actions">${renderSelectionControl(itemKey, isSelected, context, addDisabled)}</div>`,
    rowClass,
  };
}
...
function renderWineRow(wine: Wine, context?: CatalogRenderContext): string {
  ...
  const addDisabled = wine.price === null;
  const { actionsHtml, rowClass } = buildRowSelection(itemKey, isSelected, context, addDisabled);
...
function renderSpiritRow(spirit: Spirit, context?: CatalogRenderContext): string {
  ...
  const isSelected = context ? context.selectedKeys.has(itemKey) : false;
  const addDisabled = spirit.price === null;
  const { actionsHtml, rowClass } = buildRowSelection(itemKey, isSelected, context, addDisabled);
  ...
  return `
    <section class="country-section">
      <header class="country-section__header">
        ...
        <span class="country-section__count">${escapeHtml(formatSpiritCategoryCount(categoryCount))}</span>
      </header>
      <div class="country-section__regions">
        ${categories}
      </div>
